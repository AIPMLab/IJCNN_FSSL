...
class CNNMedical(nn.Module):
    def __init__(self, num_classes=4):
        super(CNNMedical, self).__init__()
        # 卷积层
        self.conv_layer = nn.Sequential(
            nn.Conv2d(in_channels=3, out_channels=32, kernel_size=3, padding=1),
            nn.BatchNorm2d(32),
        ...
        # 自适应池化层，将特征图调整为固定尺寸（7x7）
        self.adaptive_pool = nn.AdaptiveAvgPool2d((7, 7))
        # 全连接层
        self.fc_layer = nn.Sequential(
            nn.Dropout(p=0.1),
            nn.Linear(256 * 7 * 7, 1024),  # 输入尺寸是 256 * 7 * 7
            nn.ReLU(inplace=True),
            nn.Linear(1024, 512),
            nn.ReLU(inplace=True),
            nn.Dropout(p=0.1),
...
def get_model(name, backbone):
    if name == 'local':
        ...
        model = CNNCifar().to('cuda')
    elif name == 'fedfixmatch' and backbone == 'Medical':
        model = CNNMedical().to('cuda')  # 使用 CNNMedical
    else:
