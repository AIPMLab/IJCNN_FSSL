...
class Medical_Transform: # 医疗数据集的增强处理
    def __init__(self):
        self.trans_medical1 = transforms.Compose([
        ...
        self.trans_medical2 = transforms.Compose([
            transforms.Resize((224, 224)),
            RandomTranslateWithReflect(4),
            transforms.RandomHorizontalFlip(),
            transforms.ToTensor(),
    ...
    def __call__(self, x):
        x1 = self.trans_medical1(x)
        x2 = self.trans_medical2(x)
        return x1, x2
...
class Medical_Transform_t():  # 医疗数据集的测试集处理
    def __init__(self):
        self.trans_medical = transforms.Compose([
            transforms.Resize((224, 224)),
            transforms.ToTensor(),
            transforms.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])  # 使用ImageNet的均值和标准差
        ])
...
def get_aug(name, train, train_classifier=True):
    if train == True:
        ...
        elif name == 'medical':  # 添加医疗数据集的增强处理
            augmentation = Medical_Transform()
        else:
        ...
        elif name == 'medical':  # 添加医疗数据集的测试集处理
            augmentation = Medical_Transform_t()
        else:
            raise NotImplementedError
...
class Medical_Transform_fedmatch:
    def __init__(self):
        self.trans_medical1 = transforms.Compose([
            transforms.Resize((224, 224)),
            transforms.RandomHorizontalFlip(),
            transforms.RandomCrop(size=224,
                                  ...
                                  padding_mode='reflect'),
            RandAugmentMC(n=15, m=10),  # 使用 RandAugmentMC
            transforms.ToTensor(),
...
def get_aug_fedmatch(name, train, train_classifier=True):
    if train:
        if name == 'medical':
            augmentation = Medical_Transform_fedmatch()
        elif name == 'mnist':
...
def get_aug_uda(name, train, train_classifier=True):
    if train:
        if name == 'medical':
            augmentation = Medical_Transform_uda()
        elif name == 'mnist':
